...
fun StockChart(
    ...
    Canvas(modifier = modifier) {
        ...
        val priceStep = (uppperValue - lowerValue) / 5f
        (0..4).forEach { i ->
            drawContext.canvas.nativeCanvas.apply {
                drawText(
        ...
        val stokePath = Path().apply {
            ...
            for (i in infos.indices) {
                ...
                val leftRatio = (info.close - lowerValue) / (uppperValue - lowerValue)
                val rightRatio = (nextInfo.close - lowerValue) / (uppperValue - lowerValue)
        ...
        val fillPath = android.graphics.Path(stokePath.asAndroidPath())
            .asComposePath()
            .apply {
                lineTo(lasatX, size.height - spacing)
